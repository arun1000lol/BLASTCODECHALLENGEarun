import fs from 'fs';
import { MatchOverview, Player, Round } from '../types/interfaces';

export class CSGOLogParser {
  private filePath: string;
  private matchData: Partial<MatchOverview> = {};
  private players = new Map<string, Partial<Player>>();
  private rounds: Round[] = [];
  private currentRound: Partial<Round> | null = null;
  ...
  public async parseFile(): Promise<MatchOverview> {
    ...
    return new Promise((resolve, reject) => {
      ...
      stream.on('end', () => {
        ...
        if (this.currentRound) {
          this.rounds.push(this.currentRound as Round);
        }
        ...
        resolve(this.matchData as MatchOverview);
      });
  ...
  private processRegexLine(line: string): void {
  ...
  private processUtilThrown(): void {
  ...
  private processKillsAndDamage(): void {
    for (const kill of this.killEvents) {
      ...
      // track weapon usage
      if (!killer.weapons) killer.weapons = {};
  ...
  private fixPlayerTeams(): void {
    if (!this.matchData.teams) return;
    ...
    for (const player of naviPlayers.values()) {
      if (player) {
        this.matchData.teams.navi.players.push(player as Player);
      }
    }
    
    for (const player of vitalityPlayers.values()) {
      if (player) {
        this.matchData.teams.Vitality.players.push(player as Player);
      }
    }
  ...
  private getTimestamp(line: string): string | null {
  ...
  private parseTime(ts: string): Date | null {
    if (!ts) return null;
  ...
  private getAverageRoundTime(): string {
